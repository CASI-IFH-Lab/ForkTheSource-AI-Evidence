...
## 6:00 — P7 MERGED
branch: ritik/p7-id-guard -> main @ 6d50689
tests: 544 passed, 1 skipped
publishes: extract_references(doc, config=None, client=None, notes=None) -> ExtractionResult - notes is an OPTIONAL trailing append-only list, the same shape as resolve(ref, notes=); the return type is UNCHANGED so `refs, malformed = extract_references(doc)` still works. src.pipeline.last_run_notes: list[str] (what the most recent run() had to correct; overwritten per run). scripts/precache_demo.py [pdf] [--check-offline].
notes: D-109 - after a reply validates and before it is returned, a doi or arxiv_id the model supplied that does NOT appear in the entry's printed raw_text is nulled, noted and warned. Comparison is provenance not format: casefolded, ALL whitespace removed (pdfplumber renders a real PLOS DOI as "10. 1016/j.ajpath..."), prefixes arXiv:/doi:/https://doi.org//abs//CoRR abs/ and a trailing vN stripped. NOT marked malformed - D-102 means the extraction attempt produced nothing usable, and this is a good extraction with one bad field. `year` deliberately NOT guarded: it cannot cause a wrong resolution and a containment test over it fires on correct extractions. MEASURED: 1 identifier nulled across all 104 refs, 0 remaining; cache VERIFIED warm (125 stored replies before and after re-running all three papers) rather than assumed; paper1 R24 conflict -> verified; injected defects by status 4/6 -> 5/6; clean-control gate still PASSES; 74-ref baseline UNMOVED (sample 23/4/0/13, plos 17/6/2/9). Offline proven with sockets blocked, which is stronger than Wi-Fi off: OFFLINE RUN OK 2.64s, identical counts.
next: standing by for the demo; Phase 2 row 1 is whatever the dress run exposed.
...
## 6:00 — REQUEST -> @roy
Three things off the dress run, all yours to decide, none of them changed by me.
1. RE-NARRATE ADDENDUM beat 1:20. Scripted as "no record found"; the screen shows a RESOLVED RECORD. R21 renders "As printed: Entropy-based regularisation... Lindner/Vasquez/Ohara 2022" beside "Resolved record: A Robust and High-Dimensional Clustering Algorithm... Xinzhi Du 2023 10.3390/e25030510", signals 0.64 / 0.00. The rationale is right; the script is wrong. Suggested: "we searched by title - this is the nearest thing in the registry, 0.64 similar, no author in common. We are not telling you the citation is wrong. We are telling you we could not find it."
2. R11 (D01, swapped DOI) can NEVER emit doi_mismatch as defined. The defect swaps in a real other DOI; we resolve BY that DOI, so printed and resolved agree, doi_match is True, and doi_mismatch requires them to differ. Status conflict is correct (title 0.08, no shared author). Either the label expects an indicator the pipeline cannot produce, or doi_mismatch needs a second meaning. Your label, P5's semantics - not changing either now.
3. R24 (D20) still emits no version_mismatch. D-109 fixed the false conflict and the status is verified as labelled, but with the invented arXiv id gone the reference no longer looks like a preprint on our side, so there is nothing to compare. Trap reads 0/1.
