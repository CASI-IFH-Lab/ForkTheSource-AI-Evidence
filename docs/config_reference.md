...
3. **Three constants in the priority formula are not in config.** The formula is
   `severity × min(1.0, 0.4 + 0.2 × citing_claims) × confidence`, `+0.3` if `retracted`,
   capped at `1.0`. Only `severity` is configurable. The `0.4`/`0.2` claim weighting, the
   `0.3` retraction bonus and the cap will end up hardcoded in whichever of P6 or A1 lands
   first. **Not added here deliberately** — inventing key names for them would pre-empt a
   decision that belongs to P6, and the plan itself says the formula "lives here or A1".
   Worth five minutes at Sync 1.
